...
Stacks: MI355X = merged phase4 (Codex phase3 + ws1 decode MoE CSV + ws3 fused
AR+norm; PTPC OFF per accuracy policy; vLLM ac750, aiter 0.1.19). GB300 =
canonical production path (FLASHINFER+trtllm, EAGLE3). D protocol: TP4,
DecodeBenchConnector, EAGLE3 draft-3 synthetic, 60-120K ISL, OSL 600.
SLA: TPOT p50 <= 16.6 ms.

### D-only (latest)

| Platform | Best point | out tok/s | TPOT p50 | QPS |
|---|---|---:|---:|---:|
| GB300 (NV) | C72 | **3,991.1** | 16.39 ms | ~6.7 |
| MI355X phase4 (AMD) | **C28 (sweet spot)** | 1,559.7 | 16.35 ms | **2.60** (closed-loop) |
| MI355X phase4 open-loop | 2.4 QPS | 1,423.6 | 15.72 ms | 2.37 achieved, PASS |

NV/AMD at best-under-SLA: **2.56x** throughput (3,991/1,560). AMD TPOT improved
33.2 -> 16.35 ms (-51%) across the optimization campaign; the gap is now
dominated by dense attention (18.1x per-call) and MoE (1.50x); AR+norm and
projections are at parity or AMD-faster.

Kernel detail: see the C24 per-kernel table in the historical section (same
stack; C24 geometry: MI verify graph 37.7 ms/step — MoE 39.6%, dense attn
24.3%, indexer 7.0%; GB300 graph 29.6 ms — dense attn only 0.51 ms/step).

### P-only (latest)

| Platform | SLA sweet spot |
|---|---:|
| GB300 | 44,135 fresh tok/s (C8, 60-120K ISL, ~90% prefix reuse) |
| MI355X phase3 | 35,173 tok/s (+10.8% over its baseline; 89% of ATOM) |
| ATOM reference | 39.5K tok/s |

NV/AMD P: **1.26x**. P kernel history is in the historical chapter below.

---

## Kernel detail backing the Latest chapter (C24 geometry, 2026-08-28)
